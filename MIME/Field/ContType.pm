...
sub charset {
    my $value = shift->param('charset', @_);
    if (!defined($value) || ($value eq '')) {
	return 'us-ascii';               # RFC-1521
    }
    lc($value);       # returns all lowercase
}
sub id {
    my $value = shift->param('id', @_);
    defined($value) ? $value : '';
}
sub name {
    my $value = shift->param('name', @_);
    defined($value) ? $value : '';
}
sub number {
    my $value = shift->param('number', @_);
    defined($value) ? $value : '';
}
sub total {
    my $value = shift->param('total', @_);
    defined($value) ? $value : '';
}
...
sub type {
    ...
    # Trap for missing value:
    if (!defined($value) || ($value eq '')) {
	return 'text/plain';               # RFC-1521
    }
